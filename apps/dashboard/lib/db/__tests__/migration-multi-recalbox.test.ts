...
describe('multi-recalbox migration', () => {
	it('creates default Recalbox from settings and backfills sessions', () => {
		...
		sqlite.prepare(`INSERT INTO sessions (recalbox_id, started_at, system, rom_path) VALUES (NULL, ?, 'snes', '/rom/test.zip')`).run(Math.floor(now / 1000))

		// Simulate migration inline
		const settings = Object.fromEntries(
			(sqlite.prepare('SELECT key, value FROM settings').all() as { key: string; value: string }[]).map((r) => [r.key, r.value])
		)
		...
		sqlite.prepare(`INSERT INTO recalboxes VALUES (?, 'My Recalbox', ?, ?, ?, 22, 1883, NULL, NULL, 1, 0, ?, NULL)`)
			.run(defaultId, host, settings['recalbox.sshUser'] ?? 'root', settings['recalbox.sshPassword'] ?? '', now)
	...
	it('is idempotent: does not create duplicate Recalbox on second run', () => {
		const sqlite = makeDb()
		const now = Date.now()
		sqlite.prepare(`INSERT INTO recalboxes VALUES ('existing-id', 'My Recalbox', 'recalbox.local', 'root', '', 22, 1883, NULL, NULL, 1, 0, ?, NULL)`).run(now)
		sqlite.prepare(`INSERT INTO settings VALUES ('__multi_recalbox_migrated__', 'true', ?)`).run(now)

		const flag = sqlite.prepare(`SELECT value FROM settings WHERE key = '__multi_recalbox_migrated__'`).get() as { value: string } | undefined
	...
	it('preserves existing data: session count unchanged after migration', () => {
		...
		for (let i = 0; i < 5; i++) {
			sqlite.prepare(`INSERT INTO sessions (recalbox_id, started_at, system, rom_path) VALUES (NULL, ?, 'snes', ?)`).run(i, `/rom/${i}.zip`)
		}
		const id = 'migrated-id'
		sqlite.prepare(`INSERT INTO recalboxes VALUES (?, 'R', 'box.local', 'root', 'pass', 22, 1883, NULL, NULL, 1, 0, ?, NULL)`).run(id, now)
		...
		expect(count).toBe(5)
		const allLinked = sqlite.prepare(`SELECT COUNT(*) as c FROM sessions WHERE recalbox_id = ?`).get(id) as { c: number }
