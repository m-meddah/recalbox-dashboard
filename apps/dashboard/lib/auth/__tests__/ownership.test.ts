...
const getRecalboxes = vi.fn()
vi.mock('@/lib/config-store', () => ({ configStore: { getRecalboxes: () => getRecalboxes() } }))
...
afterEach(() => getRecalboxes.mockReset())
...
describe('getViewableRecalboxIds', () => {
	it('returns all recalboxes for an admin', () => {
		getRecalboxes.mockReturnValue(rows())
