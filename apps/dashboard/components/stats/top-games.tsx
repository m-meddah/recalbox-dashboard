...
export function TopGames({ games }: Props) {
  ...
  if (games.length === 0) {
    return (
      <p className="py-4 text-center text-sm text-muted-foreground">
        Aucun jeu joué sur cette période
      </p>
    )
  }
  ...
  return (
    <div className="space-y-1">
      {visible.map((game, i) => (
        <div key={game.romPath} className="group flex items-center gap-3 rounded-lg px-2 py-1.5 hover:bg-muted/50">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-1 flex items-center gap-2">
              ...
              <span className="shrink-0 text-[10px] text-muted-foreground capitalize">{game.system}</span>
            </div>
          </div>
        </div>
      ))}
      ...
      {games.length > 10 && (
        ...
        >
          {showAll ? 'Voir moins' : `Voir plus (${Math.min(50, games.length) - 10} jeux)`}
        </button>
      )}
    </div>
  )
