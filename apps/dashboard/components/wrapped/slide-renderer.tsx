...
export function SlideRenderer({ slide, wrapped, slideIndex }: Props) {
	switch (slide.type) {
		case 'intro':
			return <IntroSlideView slide={slide} wrapped={wrapped} />
		case 'total-time':
			return <TotalTimeSlideView slide={slide} />
		case 'most-played-game':
			return <MostPlayedGameSlideView slide={slide} />
		case 'top-system':
			return <TopSystemSlideView slide={slide} />
		case 'top-games-list':
			return <TopGamesListSlideView slide={slide} />
		case 'longest-session':
			return <LongestSessionSlideView slide={slide} />
		case 'busiest-day':
			return <BusiestDaySlideView slide={slide} />
		case 'streak':
			return <StreakSlideView slide={slide} />
		case 'achievements-summary':
			return <AchievementsSlideView slide={slide} />
		case 'unlocks':
			return <UnlocksSlideView slide={slide} />
		case 'comparison-vs-others':
			return <ComparisonSlideView slide={slide} />
		case 'outro':
			return <OutroSlideView slide={slide} shareSlideIndex={slideIndex} />
		default:
			return null
	}
}
